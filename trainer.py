...
def Sin(Z):
    # return 1 / (1 + np.exp(-Z)) + np.tanh(Z)
    # return 1 / (1 + np.exp(-Z)) 
    return np.tanh(Z) 

def Sin_deriv(Z):
    # sigmoid_z = 1 / (1 + np.exp(-Z))
    # return sigmoid_z * (1 - sigmoid_z) + 1 - np.tanh(Z)**2
...
def backward_prop(Z1, A1, Z2, A2, W1, W2, X, Y):
    ...
    dZ2 = A2 - one_hot_Y
    # dZ2 = one_hot_Y * np.log(-A2)
    dW2 = 1 / m * dZ2.dot(A1.T)
    db2 = 1 / m * np.sum(dZ2)

    # dZ1 = W2.T.dot(dZ2) * ReLU_deriv(Z1)
    dZ1 = W2.T.dot(dZ2) * Sin_deriv(Z1)
    dW1 = 1 / m * dZ1.dot(X.T)
    db1 = 1 / m * np.sum(dZ1)
...
def give_prediction(index, W1, b1, W2, b2):
    ...
    current_image = current_image.reshape((28, 28)) * 255
    plt.gray()
    plt.title(f"Index: {index}, Prediction: {prediction}, Actual: {label}")
    plt.imshow(current_image, interpolation='nearest')
    plt.show()



W1, b1, W2, b2 = gradient_descent(X_train, Y_train, 0.7, 150)
